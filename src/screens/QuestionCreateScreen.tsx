...
import type { Difficulty, QuestionType, Category } from '../types/database';

type QuestionCreateScreenProps = {
  navigation: NativeStackNavigationProp<MainStackParamList, 'QuestionCreate'>;
  route: RouteProp<MainStackParamList, 'QuestionCreate'>;
};
...
export default function QuestionCreateScreen({ navigation, route }: QuestionCreateScreenProps) {
  ...
  const loadCategories = async () => {
    try {
      const { categories: cats, error } = await questionAdminService.getCategories();
      if (error) {
  ...
  const resetForm = () => {
    ...
    setTeachingNotes('');
    setTags('');
    // Keep category, difficulty, and question type for faster entry
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      const { question, error } = await questionAdminService.createDevQuestion(questionData);
    ...
    >
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Add a new question to dev environment</Text>
        </View>
